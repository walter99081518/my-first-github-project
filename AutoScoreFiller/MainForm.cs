...
namespace AutoScoreFiller
{
    ...
    public partial class MainForm : Form
    {
        ...
        private void ParseConfigXml()
        {
            ...
            {
                classnames.Add(nodelist[i].InnerText.ToLower());
            }
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    this.msgbox.ShowMessageDialog("请选择要导入数据的表格", "错误");
                    return;
                }
        ...
        private void btnClearTable_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.lvi == null || this.lvi.SubItems[0].Text.Trim() == "")
                {
                    this.msgbox.ShowMessageDialog("请选择要清空数据的表格", "错误");
                    return;
                }
        ...
        private void lvTableShow_ColumnClick(object sender, ColumnClickEventArgs e)
        {
            if (e.Column != 0)
            {
                return;
        ...
        private void lvTableShow_MouseMove(object sender, MouseEventArgs e)
        {/*
            ListView lv = (ListView)sender;
            ListViewItem oldlvi = null;
            string tblId = null;
            HtmlElement he = null;
            if (lv.Tag != null) oldlvi = (ListViewItem)lv.Tag;

            ListViewItem lvi = lv.GetItemAt(e.X, e.Y);
            if (lvi != null)
            {
                if (oldlvi != null && !oldlvi.Equals(lvi) && !oldlvi.Selected)
                {
                    oldlvi.BackColor = lv.BackColor;
                    oldlvi.ForeColor = Color.Black;
                    tblId = oldlvi.SubItems[0].Text;
                    he = this.webBrowser.Document.GetElementById(tblId);
                    if (he != null)
                    {
                ...
                if (!lvi.Selected)
                {
                    ...
                    tblId = lvi.SubItems[0].Text;
                    he = this.webBrowser.Document.GetElementById(tblId);
                    if (he != null)
                    {
                        he.Style = "background-color: #DCB5FF";
                    }
                }

                lv.Tag = lvi;
            }
            else // 鼠标移动到没有ListViewItem的区域
            {
                if (oldlvi != null && !oldlvi.BackColor.Equals(lv.BackColor) && !oldlvi.Selected)
                {
                    oldlvi.BackColor = lv.BackColor;
                    oldlvi.ForeColor = Color.Black;
                    tblId = oldlvi.SubItems[0].Text;
        ...
        private void lvTableShow_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            HtmlElement he = null;
            if (this.lvi != null && !e.IsSelected)
            {
                he = this.webBrowser.Document.GetElementById(this.lvi.SubItems[0].Text);
                if (he != null)
                {
                    he.Style = "background-color : none";
                    this.lvi.BackColor = this.lvTableShow.BackColor;
                    this.lvi.ForeColor = Color.Black;
                }
                this.lvi = null;
                return;
            }
            ...
            he = this.webBrowser.Document.GetElementById(e.Item.SubItems[0].Text);
            if (he != null)
            {
                he.Style = "background-color : #DCB5FF";
            }
        ...
        private void findTable()
        {
            try
            {
                ...
                for (int i = 0; hec != null && i < hec.Count; i++)
                {
                    ...
                    this.lvTableShow.Items.Add(lvi);
                }
        ...
        private void MenuItemFindTable_Click(object sender, EventArgs e)
        {
            this.findTable();
        }
        ...
        private void MenuItemClearEditorSelector_Click(object sender, EventArgs e)
        {
            this.btnClearTable_Click(sender, e);
        }
    }
}
